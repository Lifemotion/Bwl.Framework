...
Public Class RemoteAppServer
    ...
    Private ReadOnly _remoteAddress As String
    Private ReadOnly _remoteUser As String
    Private ReadOnly _remotePassword As String
    Private ReadOnly _asyncReset As AsyncResetEvent
    ...
    Public Sub New(remoteAddress As String, remoteUser As String, remotePassword As String, storage As SettingsStorage, logger As Logger, ui As IAutoUI)
        ...
        _task = ConnectThreadSub()
    End Sub

    Private Async Function ConnectThreadSub() As Task
        Do While Not _asyncReset.IsSet
            Await _asyncReset.WaitAsync(1000).ConfigureAwait(False)
            Try
                If Not _MessageTransport.IsConnected Then
    ...
    Private Shared Function GetPortFromAddress(address As String) As Integer
        ...
        If parts.Length <> 2 Then
            Throw New Exception("Address has wrong format! Must be hostname:port")
        End If

        Dim port As Integer
        If Not Integer.TryParse(parts(1), port) Then
            Throw New Exception("Address has wrong format! Must be hostname:port")
        End If
