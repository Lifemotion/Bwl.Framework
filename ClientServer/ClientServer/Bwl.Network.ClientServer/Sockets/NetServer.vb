...
Public Class NetServer
    ...
    Public Sub StartServer(ByVal incomingPort As Integer) Implements IMessageServer.StartServer
        ...
        Try
            ...
            working = True
            directOnly = False
            _cleanerTask = OldConnectionCleaner()
        Catch ex As Exception
            tcpListener = Nothing
            working = False
    ...
    Public Sub Dispose() Implements IMessageTransport.Dispose
        ...
        If _cleanerTask IsNot Nothing Then
            _cleanerTask.Wait(2000) ' Give it 2 seconds to finish
            _cleanerTask = Nothing
        End If
    End Sub
    ...
    Private Async Function OldConnectionCleaner() As Task
        While IsWorking
            SyncLock _syncRoot
                For Each client As ClientData In connectedClients
                    If (DateTime.UtcNow - client.LastActivity).TotalSeconds > 60 Then
