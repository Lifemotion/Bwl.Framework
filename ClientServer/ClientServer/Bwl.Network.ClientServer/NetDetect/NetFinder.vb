...
Public Class NetFinder
    ...
    Public Shared Async Function Find(timeout As Integer, Optional port As Integer = 19999) As Task(Of NetBeaconInfo())
        Dim list As New List(Of NetBeaconInfo)
        FindFiles(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Temp", "BwlNetBeacons"), list)
        FindFiles(Path.Combine(Path.GetTempPath(), "BwlNetBeacons"), list)
        Try
            Await FindNet(list, timeout, port)
    ...
    Private Shared Async Function FindNet(list As List(Of NetBeaconInfo), timeout As Integer, port As Integer) As Task
