...
namespace Bwl.Network.ClientServer.Windows
{
    ...
    public partial class SettingsClient : BaseClient, ISettingsStorageForm, IDisposable
    {
        ...
        private void SettingChangeErrorHandler(object? sender, (string settingName, string errorName) values)
        {
            if (_settingsForm is not null)
                _settingsForm.Invoke(() => _settingsForm.Close());
            MessageBox.Show("Error", "Setting [" + values.settingName + "] save error: " + values.errorName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
