...
namespace Bwl.Framework.Avalonia
{
    public partial class DatagridLogWriter : UserControl, ILogWriter
    {
        ...
        public DatagridLogWriter()
        {
            ...
            logDataGrid.ItemsSource = messages;
            ...
            lock (_newMessagesListLock)
            {
                if (newMessages.Count > 0)
                {
                    lock (_gridSync)
                    {
                        ...
                        {
                            // Scroll to last item
                            if (messages.Count > 0) logDataGrid.ScrollIntoView(messages[messages.Count - 1], null);
                        }
                    }
                }
            }
        ...
        {
            if (logDataGrid.SelectedItem is LogItem selectedItem)
            {
        ...
        {
            logDataGrid.Columns[1].IsVisible = cbExtended.IsChecked.Value;
            logDataGrid.Columns[2].IsVisible = cbExtended.IsChecked.Value;
            logDataGrid.Columns[4].IsVisible = cbExtended.IsChecked.Value;
        }
