...
Friend Class EmbDatagridLogWriter
    ...
    End Sub
    Public Sub ConnectedToLogger(logger As EmbLogger) Implements ILogWriter.ConnectedToLogger
    ...
    Private Function Filter(message As ListItem) As Boolean
        With message
            ...
            Else
                textFilter = True
                Dim parts = tbFilter.Text.Split(",")
                Dim path = .path.Aggregate(Function(f, t) $"{f}.{t}")
                For Each part In parts
                    Dim found As Boolean = False
                    part = part.Trim()
                    If part = "" Then found = True
                    If .message.ToLower().Contains(part.ToLower()) Then found = True
                    If path.ToLower().Contains(part.ToLower()) Then found = True
                    If .type.ToString().Contains(part.ToLower()) Then found = True
    ...
    Private Sub grid_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles grid.CellDoubleClick
        ...
        If (e.RowIndex >= 0 AndAlso e.RowIndex <= grid.RowCount) Then
            Dim LogInfo = New EmbFormLogInfo()
