...
namespace Bwl.Framework.Test.CLI
{
    internal class Program
    {
        ...
        private static readonly AsyncResetEvent _asyncReset = new AsyncResetEvent(false);
        ...
        /// </summary>
        private async static Task LogSettingsValue()
        {
            try
            {
                while (!_asyncReset.IsSet)
                {
                    ...
                    await _asyncReset.WaitAsync(5000).ConfigureAwait(false);
                }
            }
            catch
            {
