...
Imports System.Text
Imports Bwl.Framework.SharedMemory
...
Public Class FrameworkTests
    ...
    Public Sub MicroLoggerTest()
        ...
        Dim fullLogPath = Path.GetFullPath(Path.Combine(logPath, logFileName))
        ...
        For Each updateDelayMs In {10, 100, 1000}
            ...
            For Each line In File.ReadAllLines(fullLogPath)
    ...
    Public Interface ITestClass
        Function ProcessData(data As Byte()) As String
        Sub SaveData(value As String)
        Function RetrieveData() As String
        Function AcceptData(data As Byte()) As Boolean
        Function InvertBytes(data As Byte()) As Byte()
        Function AcceptDataAsync(data As Byte()) As Task(Of Boolean)
    End Interface
    ...
    Public Class TestClass
        ...
        Public Function ProcessData(data As Byte()) As String Implements ITestClass.ProcessData
            Return Encoding.UTF8.GetString(data)
        End Function

        Public Sub SaveData(value As String) Implements ITestClass.SaveData
            _data = value
        End Sub

        Public Function RetrieveData() As String Implements ITestClass.RetrieveData
            Return _data
        End Function


        Public Function InvertBytes(data As Byte()) As Byte() Implements ITestClass.InvertBytes
            Dim resultData = New Byte(data.Length - 1) {}
            For i As Integer = 0 To data.Length - 1
                resultData(i) = CByte(data(i) Xor &HFF)
            Next
            Return resultData
        End Function

        Public Function AcceptData(data As Byte()) As Boolean Implements ITestClass.AcceptData
            Return data.Any()
        End Function

        Public Async Function AcceptDataAsync(data As Byte()) As Task(Of Boolean) Implements ITestClass.AcceptDataAsync
    ...
    <Test> <Parallelizable(ParallelScope.Self)>
    Public Sub SharedMemoryRPCInterfaceTest()

        Dim origString = "Test data for SharedMemoryRPCInterfaceTest"
        Dim testData = Encoding.UTF8.GetBytes(origString)

        Dim testClient = SharedMemoryRPCClient(Of ITestClass).Create("TestClass")

        ' First attempt
        Using testServer = New SharedMemoryRPCServer(Of ITestClass)(New TestClass(), "TestClass")
            Dim result = testClient.ProcessData(testData)
            Assert.That((result = origString), "TestData did not decode correctly!")
        End Using

        ' Second attempt - client is still active, btw
        Using testServer = New SharedMemoryRPCServer(Of ITestClass)(New TestClass(), "TestClass")
            Dim result = testClient.ProcessData(testData)
            Assert.That((result = origString), "TestData did not decode correctly!")
        End Using

        ' Third attempt - and client is still active!
        Using testServer = New SharedMemoryRPCServer(Of ITestClass)(New TestClass(), "TestClass")
            Dim result = testClient.ProcessData(testData)
            Assert.That((result = origString), "TestData did not decode correctly!")
        End Using

        ' Testing other funcitonality
        Using testServer = New SharedMemoryRPCServer(Of ITestClass)(New TestClass(), "TestClass")
            testClient.SaveData(origString)
            Dim result = testClient.RetrieveData()
            Assert.That((result = origString), "Server did not save the data!")
        End Using

    End Sub

    <Test> <Parallelizable(ParallelScope.Self)> <TestCase(2)> <TestCase(8)> <TestCase(30)>
    Public Sub SharedMemoryRPCTest(sizeMb As Integer)
        Dim dataSize As Integer = 1024 * 1024 * sizeMb
        Dim iterations As Integer = 100
        Dim totalBytesTransferred As Long = CLng(dataSize) * iterations

        Dim rnd As New Random()
        Dim testDataArray(iterations - 1)() As Byte
        ' Fill the test data array with random bytes
        For i = 0 To iterations - 1
            testDataArray(i) = New Byte(dataSize - 1) {}
            rnd.NextBytes(testDataArray(i))
        Next

        Dim testClient = SharedMemoryRPCClient(Of ITestClass).Create("SharedMemoryRPCTest")
        Using testServer = New SharedMemoryRPCServer(Of ITestClass)(New TestClass(), "SharedMemoryRPCTest")

            Dim sw As New Stopwatch()
            sw.Start()
            For i = 0 To iterations - 1
                Try
                    Dim result = testClient.InvertBytes(testDataArray(i))
            ...
            Dim throughput As Double = totalBytesTransferred / sw.Elapsed.TotalSeconds
            Console.WriteLine("Throughput: {0:N0} bytes/second", throughput)
            Console.WriteLine("Throughput: {0:N2} megabytes/second", throughput / 1024 / 1024)

        End Using
    End Sub

    <Test> <Parallelizable(ParallelScope.Self)> <TestCase(2)> <TestCase(8)> <TestCase(30)>
    Public Sub SharedMemoryBinaryTestByteArray(sizeMb As Integer)
        Dim dataSize As Integer = 1024 * 1024 * sizeMb
        Dim iterations As Integer = 100
        Dim totalBytesTransferred As Long = CLng(dataSize) * iterations
        ...
        Dim testClient = New SharedMemoryBinaryClient("SharedMemoryBinaryTestByteArray")

        Using testServer = New SharedMemoryBinaryServer(AddressOf InvertBytesByteArray, "SharedMemoryBinaryTestByteArray")
            Dim sw As New Stopwatch()
            sw.Start()
            For i = 0 To iterations - 1
                Try
                    Dim result = testClient.ProcessData(testDataArray(i))
                Catch ex As Exception
                    ' Do nothing, just catch the exception
                End Try
            Next
    ...
    <Test> <Parallelizable(ParallelScope.Self)> <TestCase(2)> <TestCase(8)> <TestCase(30)>
    Public Sub SharedMemoryBinaryTestPointer(sizeMb As Integer)
        Dim dataSize As Integer = 1024 * 1024 * sizeMb
        Dim iterations As Integer = 100
        Dim totalBytesTransferred As Long = CLng(dataSize) * iterations
        ...
        Dim testClient = New SharedMemoryBinaryClient("SharedMemoryBinaryTestPointer")

        Using testServer = New SharedMemoryBinaryServer(AddressOf InvertBytesPointer, "SharedMemoryBinaryTestPointer")
            Dim sw As New Stopwatch()
            sw.Start()
            For i = 0 To iterations - 1
                Try
    ...
    Private Shared Function InvertBytesByteArray(data As Byte()) As Byte()
        Return data.Select(Function(f) CByte(f Xor &HFF)).ToArray()
    End Function

    Private Shared Function InvertBytesPointer(data As IntPtr, payloadSize As Integer) As Integer
        For i As Integer = 0 To payloadSize - 1
            Dim b As Byte = Marshal.ReadByte(data, i)
            ' Bitwise invert b using XOR with 255
            Marshal.WriteByte(data, i, CByte(b Xor &HFF))
        Next
        Return payloadSize
    End Function
